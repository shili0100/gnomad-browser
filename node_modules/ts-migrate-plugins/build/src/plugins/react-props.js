...
const reactPropsPlugin = {
    ...
    run({ fileName, sourceFile, options }) {
        ...
        if (!fileName.endsWith('.tsx'))
            return undefined;
        const updates = [];
        const getPropsTypeName = (0, react_props_1.createPropsTypeNameGetter)(sourceFile);
        ...
        const updatedSourceText = (0, updateSourceText_1.default)(sourceFile.text, updates);
        ...
        const importUpdates = !options.shouldKeepPropTypes
            ? (0, imports_1.updateImports)(updatedSourceFile, spreadReplacements.map((cur) => cur.typeImport), [
            ...
            : [];
        return (0, updateSourceText_1.default)(updatedSourceText, importUpdates);
    },
...
function isReactNode(node) {
    return ((typescript_1.default.isClassDeclaration(node) && (0, react_1.isReactClassComponent)(node)) ||
        (typescript_1.default.isFunctionDeclaration(node) && (0, react_1.isReactSfcFunctionDeclaration)(node)) ||
        (typescript_1.default.isVariableStatement(node) && (0, react_1.isReactSfcArrowFunction)(node)));
}
...
function updatePropTypes(node, propsTypeName, sourceFile, propTypeIdentifiers, options) {
    ...
    if (isReactSfcNode(node)) {
        ...
        const heritageType = (0, react_1.getReactComponentHeritageType)(node);
...
function updateObjectLiteral(node, objectLiteral, propsTypeName, sourceFile, propTypeIdentifiers, options, implicitChildren) {
    const updates = [];
    const printer = typescript_1.default.createPrinter();
    const propsTypeNode = (0, react_props_1.default)(objectLiteral, sourceFile, {
    ...
    if (varStatement && !options.shouldKeepPropTypes) {
        updates.push({
            ...
            text: (0, text_1.getTextPreservingWhitespace)(varStatement, propsTypeAlias, sourceFile),
        });
    }
    else {
        updates.push({
...
function getReactForwardRefFuncExpression(node) {
    ...
    return (!!typescript_1.default.isVariableStatement(node) &&
        ...
        (0, react_1.isReactForwardRefName)((_f = (_e = node.declarationList) === null || _e === void 0 ? void 0 : _e.declarations[0]) === null || _f === void 0 ? void 0 : _f.initializer) &&
