...
function getTextWithIgnores(sourceFile, diagnostics, options) {
    ...
    diagnostics.forEach((diagnostic) => {
        ...
        const tsIgnoreCommentText = `@${errorExpression} TS(${code})${messagePrefixInComment}: ${message.length > messageLimit
...
function inJsxText(sourceFile, pos) {
    const visitor = (node) => {
        if (node.pos <= pos && pos < node.end && (typescript_1.default.isJsxElement(node) || typescript_1.default.isJsxFragment(node))) {
            ...
            const isClosingElement = !(0, typescript_1.isJsxFragment)(node) && node.closingElement.pos === pos;
