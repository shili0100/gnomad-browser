...
exports.fetchVariantSearchResults = exports.fetchSearchResults = void 0;
...
const fetchSearchResults = (datasetId, query) => {
    if (datasetId.startsWith('gnomad_sv')) {
        // ==============================================================================================
        // Structural Variants
        // ==============================================================================================
        if ((0, identifiers_2.isStructuralVariantId)(query, datasetId)) {
        ...
        // Variants
        // ==============================================================================================
        if ((0, identifiers_1.isVariantId)(query)) {
            const variantId = (0, identifiers_1.normalizeVariantId)(query);
            return Promise.resolve([
                {
                    label: variantId,
        ...
        if ((0, identifiers_1.isRsId)(query)) {
            const rsId = query;
            return Promise.resolve([
                {
                    label: rsId,
    ...
    // Region
    // ==============================================================================================
    if ((0, identifiers_1.isRegionId)(query)) {
        const { chrom, start, stop } = (0, identifiers_1.parseRegionId)(query);
    ...
    if (/^[A-Z][A-Z0-9-]*$/.test(upperCaseQuery)) {
        ...
        `,
                variables: { query, referenceGenome: (0, metadata_1.referenceGenome)(datasetId) },
            }),
            method: 'POST',
            ...
            return genes
                ...
                .map((gene) => ({
                label: geneSymbolCounts[gene.symbol] > 1
    ...
    if (/.+\s+AND\s+.+/.test(upperCaseQuery)) {
        ...
        if ((0, identifiers_1.isVariantId)(parts[0]) && (0, identifiers_1.isVariantId)(parts[1])) {
            const variantOneId = (0, identifiers_1.normalizeVariantId)(parts[0]);
            const variantTwoId = (0, identifiers_1.normalizeVariantId)(parts[1]);
            return Promise.resolve([
                {
...
exports.fetchVariantSearchResults = fetchVariantSearchResults;
