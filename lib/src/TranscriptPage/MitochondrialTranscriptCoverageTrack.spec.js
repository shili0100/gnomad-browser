...
globals_1.describe.each(datasetsWithoutCoverage)('MitochondrialTranscriptCoverageTrack with dataset %s', (datasetId) => {
    (0, globals_1.test)('has no unexpected changes', () => {
        const tree = react_test_renderer_1.default.create(react_1.default.createElement(MitochondrialTranscriptCoverageTrack_1.default, { datasetId: datasetId, transcriptId: "dummy-transcript" }));
        (0, globals_1.expect)(tree).toMatchSnapshot();
    });
});
globals_1.describe.each(datasetsWithCoverage)('MitochondrialTranscriptCoverageTrack with dataset %s', (datasetId) => {
    (0, globals_1.test)('queries with proper parameters', () => {
        setMockApiResponses({
            MitochondrialCoverageInRegion: () => ({}),
        });
        const shallowRenderer = (0, shallow_1.createRenderer)();
        shallowRenderer.render(react_1.default.createElement(MitochondrialTranscriptCoverageTrack_1.default, { datasetId: datasetId, transcriptId: "dummy-transcript" }));
        (0, globals_1.expect)(shallowRenderer.getRenderOutput()).toMatchSnapshot();
    });
});
//# sourceMappingURL=MitochondrialTranscriptCoverageTrack.spec.js.map