...
const extractPlotFragment = (tree) => tree.querySelector('svg >g:nth-child(2)');
const plotLayoutMatches = (expected, plotBody) => {
    if (plotBody === null) {
    ...
    expected.forEach((expectedElement, i) => {
        ...
        if (child.tagName !== expectedTag) {
            throw new Error(`expected tag "${expectedTag}" for element ${expectedElement} but got "${child.tagName}"`);
        }
...
(0, globals_1.describe)('ClinvarAllVariantsPlot', () => {
    globals_1.describe.each(['+', '-'])('rendering a frameshift on strand %s', (strand) => {
        const baseVariant = ClinvarVariant_1.default.build({
        ...
        const scalePosition = (...args) => args;
        const onClickVariant = (...args) => args;
        const width = 1200;
        (0, globals_1.test)('renders properly when the variant ends in the coding section', () => {
            ...
            //  as we plot it).
            const variant = Object.assign(Object.assign({}, baseVariant), { hgvsp: 'p.Tyr30SerfsTer50' });
            const tree = (0, react_2.render)(react_1.default.createElement(ClinvarAllVariantsPlot_1.default, { scalePosition: scalePosition, transcripts: [transcript], variants: [variant], width: width, onClickVariant: onClickVariant })).asFragment();
            ...
            (0, globals_1.expect)(() => plotLayoutMatches(expectedPlotLayout, plotBody)).not.toThrowError();
            (0, globals_1.expect)(tree).toMatchSnapshot();
        });
        (0, globals_1.test)('renders properly when the variant ends in the downstream UTR', () => {
            ...
            const variant = Object.assign(Object.assign({}, baseVariant), { hgvsp: 'p.Tyr30SerfsTer80' });
            const tree = (0, react_2.render)(react_1.default.createElement(ClinvarAllVariantsPlot_1.default, { scalePosition: scalePosition, transcripts: [transcript], variants: [variant], width: width, onClickVariant: onClickVariant })).asFragment();
            const plotBody = extractPlotFragment(tree);
            const plotLayouts = {
                '+': [
        ...
        (0, globals_1.test)('renders clamped to the end of the downstream UTR when the variant overruns the downstream UTR', () => {
            ...
            const variant = Object.assign(Object.assign({}, baseVariant), { hgvsp: 'p.Tyr30SerfsTer110' });
            const tree = (0, react_2.render)(react_1.default.createElement(ClinvarAllVariantsPlot_1.default, { scalePosition: scalePosition, transcripts: [transcript], variants: [variant], width: width, onClickVariant: onClickVariant })).asFragment();
            const plotBody = extractPlotFragment(tree);
            const plotLayouts = {
                '+': [
        ...
        (0, globals_1.test)('clamps end to end of the downstream UTR when the end in the HGVSP is "?"', () => {
            const variant = Object.assign(Object.assign({}, baseVariant), { hgvsp: 'p.Tyr30SerfsTer?' });
            const tree = (0, react_2.render)(react_1.default.createElement(ClinvarAllVariantsPlot_1.default, { scalePosition: scalePosition, transcripts: [transcript], variants: [variant], width: width, onClickVariant: onClickVariant })).asFragment();
            const plotBody = extractPlotFragment(tree);
            const plotLayouts = {
                '+': [
...
//# sourceMappingURL=ClinvarAllVariantsPlot.spec.js.map